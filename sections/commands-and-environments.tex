\section{Commands and Environments 命令与环境}
...
\input{listings/bilingual-table.tex}
...
\vspace{3pt}
\noindent
\OptionsShow{float}{[!]<\emph{subset of} htbp> \emph{or} <H>}{htbp}
\vspace{3pt}

\begin{ParaColumn}

    The \ColorCode{float} option defines the allowed positions of the float table, and the \verb"H" option provided by the \verb"float" package is used to create a non-floating table.

    \switchcolumn

    \ColorCode{float}选项定义浮动表格环境的允许位置，\verb"float"宏包提供的\verb"H"选项用于产生不浮动的表格。
...
\input{tables/bilingual-table-example.tex}
...
\input{listings/bilingual-figure.tex}
...
\input{figures/bilingual-figure-example.tex}
...
\input{listings/bilingual-subfigure.tex}
...
\input{figures/bilingual-subfigure-example.tex}
...
\input{listings/side-by-side-charts.tex}
...
\input{figures/side-by-side-charts-example.tex}
...
\input{listings/end-by-end-charts.tex}
...
\input{figures/end-by-end-charts-example.tex}
...
\input{figures/combination-of-side-by-side-and-end-by-end-charts.tex}
